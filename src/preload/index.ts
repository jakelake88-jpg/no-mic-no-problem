...
const api: RendererApi = {
  ...
  hfpConnect: (deviceName) => ipcRenderer.send(IPC.hfpConnect, deviceName),
  hfpDisconnect: () => ipcRenderer.send(IPC.hfpDisconnect),
  onHfpState: (cb) => subscribe(IPC.hfpState, cb),
  openSoundSettings: () => ipcRenderer.send(IPC.openSoundSettings),
  fixFirewall: () => ipcRenderer.invoke(IPC.fixFirewall),
  doctorRun: () => ipcRenderer.invoke(IPC.doctorRun),
  doctorMakePrivate: (alias) => ipcRenderer.invoke(IPC.doctorMakePrivate, alias),
